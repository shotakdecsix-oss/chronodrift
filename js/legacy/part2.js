...
// 【2026-07-20】「NYの建物が灰色で画一的」という報告への対応。office/commercial/apartment/
// industrialは種別ごとにstyle.colorが単一の固定色(例: オフィス=0x76869c)しか無く、
// tintWall(part3.js)による明暗のばらつきだけでは同じ色調の濃淡違いにしかならなかった。
// NYはbuilding=office/commercial/retailタグが非常に多く実測どおり反映されるため、
// この単一色がそのまま都心の建物の大半を占め、単調さの主因になっていた。
// 実際の高層ビル街はガラスの色味(青・緑・褐色)や石材・レンガで棟ごとに大きく違うため、
// 種別ごとに小さなパレットを用意し、addBuilding側(style.wallPalette)で棟ごとに抽選する。
const OFFICE_WALL_PALETTE = [0x76869c, 0x6a8098, 0x7a9088, 0x9a8868, 0x8a7050, 0xa8aeb4, 0x5c6870];
const APARTMENT_WALL_PALETTE = [0x8290ab, 0x8a5a4c, 0x9c8060, 0xb0a090, 0xc8bc9c, 0x707890];
const INDUSTRIAL_WALL_PALETTE = [0x808890, 0x8c9098, 0x707870, 0x9c9488];
...
function facadeMat(kind, color, variant) {
  ...
  // 【2026-07-16】窓点灯のフロア別ランダム化(縦4フロア×4種→2×2に縮小)を試したが、
...
function fitRealBuildingToRoads(cx, cz, w, d, rot) {
  ...
  const gx = Math.floor(cx / ROAD_CELL), gz = Math.floor(cz / ROAD_CELL);
  const _rails = []; // 近傍の線路セグメント(ローカル系端点+リボン半幅。最終被りチェック用)
  for (let dxc = -cellR; dxc <= cellR; dxc++) for (let dzc = -cellR; dzc <= cellR; dzc++) {
    ...
    for (const r of arr) {
      ...
      // 【2026-07-16改】線路も道路と同様に縮小対象。ただし線路は「一切被らない」方針
      // (実機報告: 駅以外でも線路への被りが目立つ。駅ビルの透過表現は廃止)なので、
      // 縮小しきれず被りが残るかを後段で最終チェックするため記録しておく。
      if (r.type === 'railway') _rails.push({ au, av, bu, bv, rhw });
  ...
  // 線路は一切の被りを許さない: 縮小後の最終寸法でもまだ重なる線路が残っていれば
  // drop=trueを返し、呼び出し側(part9.js)が生成自体をスキップする(線路またぎの駅ビル等。
  // 下限35%まで縮めても越えられない=実質線路の上に建っている建物)。
  for (const rc of _rails) {
    const du = rc.bu - rc.au, dv = rc.bv - rc.av;
    let overlap;
    if (Math.abs(du) >= Math.abs(dv)) {
      const vmin = _minAbsOverWindow(rc.au, rc.av, du, dv, hw + rc.rhw);
      overlap = vmin !== null && vmin < hd + rc.rhw;
    } else {
      const umin = _minAbsOverWindow(rc.av, rc.au, dv, du, hd + rc.rhw);
      overlap = umin !== null && umin < hw + rc.rhw;
    }
    if (overlap) return { w: hw * 2, d: hd * 2, drop: true };
  }
...
// (色タグが豊富な欧州都市などで長時間プレイすると顕著になり得る)。RGB各段階に量子化して
// からキャッシュキーに載せることで組み合わせ数の頭打ちにする。見た目の劣化はほぼ知覚不能
// (CODE_REVIEW_20260717 P12)。
// 【2026-07-20・タブクラッシュ調査】facadeCache/matCacheには一度作ったMeshLambertMaterial
// (128x128+64x64のCanvasテクスチャ2枚持ち、1組み合わせ約100〜130KB のGPUテクスチャメモリ)を
// 破棄する仕組みが無い(共有材として建物アンロード時も維持する設計のため)。16段階量子化でも
// 理論上16^3=4096通り×6ティント×kind×variantの組み合わせがあり得て、色タグが多い都市を
// 何箇所も巡る長時間セッションでは蓄積したテクスチャがGPUメモリを圧迫し、タブクラッシュの
...
function getBuildingStyle(tags) {
  ...
  if (am === 'place_of_worship' || name.includes('神社') || name.includes('shrine')) {
    ...
    // wallPalette: 棟ごとに色を抽選する(addBuilding参照)。colorは抽選が効かない
    // 場面(万一の未対応箇所)向けのフォールバック値として残す。
    style = { color: 0x76869c, wallPalette: OFFICE_WALL_PALETTE, roofColor: 0x505868, emissive: 0x0a1420, type: 'office' };
    ...
    style = { color: 0x8290ab, wallPalette: APARTMENT_WALL_PALETTE, roofColor: 0x506080, emissive: 0x001122, type: 'apartment' };
    ...
    style = { color: 0x808890, wallPalette: INDUSTRIAL_WALL_PALETTE, roofColor: 0x505860, emissive: 0x111111, type: 'industrial' };
  }
  ...
  if (wallTag != null || roofTag != null || roofShape || roofMaterial) {
    ...
    // 実測タグによる明示的な色は、種別ごとのパレット抽選より優先する(抽選に上書きされないよう外す)
    if (wallTag != null) { style.color = quantizeColor(wallTag); style.wallPalette = null; }
