...
// 2に戻す。
// 【2026-07-27・実測に基づき3→2】Overpassは1IPあたり実質2スロット(/api/status が
// rate limit=2 を報告)。RenderのegressがOverpassに到達できないことが判明し、Overpassに
// 関しては恒久的にブラウザ直アクセス(=ユーザー自身のIPで2スロット)で戦う前提になったため、
// 同時3本は構造的に必ず1本が429になる。実測でも200が4.5〜8.6秒で返る一方、3本目が429だった。
...
async function fetchOSMTileBatch(opts) {
  ...
  // 【2026-07-25修正・2026-07-27 Step Bで再度戻す】当初は3×3圏(距離1.6)だけを対象にして
  // いたが、その後スコアリング側の近傍優先(_tileScore内のNEAR_TIER_R=5×5・距離2)を先に
  // 拡張した際、こちらの1枚クエリ範囲を揃え忘れていた。結果、5×5の外周(距離2)は「優先度は
  // 最優先なのに実際のクエリは重い3枚まとめ」という不整合を抱え、そこが失敗・再試行を
  // 繰り返して3並列の枠を占有し続け、近傍のはずのタイルがいつまでも赤(fetching)のまま
  // 進まない実機報告につながったため、当時はNEAR_TIER_Rに揃えて近傍5×5丸ごとをソロ化した。
  // 【2026-07-27・IMPL_PROMPT_20260727_CONGESTION_TUNING Step B】その後tier2(3x3、
  // NEAR_SPLIT_TIER_R)とtier3(5x5の外周、NEAR_TIER_R)を別優先度帯(-10000 / -9000)に
  // 分離済みで、上記の「同列に扱われて重いまとめが3並列を占有する」前提は既に解消されている。
  // 5×5全部ソロ化は結果的に通常のエリア充填リクエスト数を約34本(3x3分離18+5x5外周ソロ16)
  // まで押し上げ、CONGESTION_BACKLOGの閾値較正とは独立にリクエスト総量そのものが渋滞の
  // 一因になっていた。tier3(外周16枚)は従来の複合3枚まとめ(OSM_TILE_BATCH)に戻し、
  // ソロ化はtier1+2(NEAR_SPLIT_TIER_R=1、3x3。道路/建物分離ジョブは元々isSplitJobで常に
  // ソロなので実質変化なし)だけに絞る。エリア充填の総リクエスト数は約34本→約24本に減る。
  const nearSolo = nextTile && Math.max(Math.abs(nextTile.tx - _pTileX), Math.abs(nextTile.tz - _pTileZ)) <= NEAR_SPLIT_TIER_R;
  ...
  //
  // 【2026-07-28・上のStep Cは実測で失敗したので撤回。ソロ固定に戻す】
  // バッチ化して再計測した結果、狙いと正反対の数字が出た:
  //     実行中の道路ジョブ '1,2|road(26.9s)'  … ソロ時の8.6sに対して約3倍
  //     B tier1 = 57.5s(ソロ時 7.8s)
  //     成功率 6/9=67%(ソロ時 11/13=85%)、429×2、クールダウン発動 23%(ソロ時4%)
  // つまりOverpassの実行時間はbbox数にほぼ線形で、3枚まとめれば1本が3倍長くなる。
  // 「本数1/3 × 1本3倍」で総処理時間は不変(実際 24本×9s と 8本×27s は同じ216秒)。
  // 得るものが無い一方、失うものが3つある:
  //   (1) 先頭ブロッキング: 優先度は実行中のジョブを追い越せないので、枠が27秒級のクエリで
  //       埋まると最優先のはずのtier1建物ジョブでも57秒待たされる(B tier1の悪化はこれ)
  //   (2) 失敗の粒度: 1本の429/504で3枚まとめて再試行に落ちる
  //   (3) 長いクエリほど429/504を踏みやすく、失敗率そのものが上がる
  // 【この計測から得た本質的な結論】律速は「リクエストの本数」ではなく
  // 【クエリする総タイル面積】。本数をどう束ねても総面積が同じなら総時間は変わらない。
  // 改善したければ取得範囲そのものか条件節を削るしかない(バッチ化・並列度は効かない)。
...
function checkOSMTiles() {
  ...
  if (flen > 1) {
    ...
    // 【2026-07-27・BIRDモード】水平移動がBIRD_SPEED_MULT倍(3倍)速いため、同じ先読み
    // 距離だと到達までの猶予時間が1/3に縮み、生成(特に建物)が追いつかない(実機報告)。
    // BIRDモード中は先読み距離(k上限)も同じ倍率で伸ばし、猶予時間を通常時と揃える
    // (幅=s方向は変えない。奥行きだけ伸ばせば十分なため)。遠方タイルはtier4扱いで
    // ベストエフォートに回るだけなので、キュー自体が伸びても既存の優先度設計は壊れない。
    const _fwdKMax = birdMode ? 6 * BIRD_SPEED_MULT : 6;
    for (let k = 3; k <= _fwdKMax; k++)
      for (let s = -1; s <= 1; s++)
