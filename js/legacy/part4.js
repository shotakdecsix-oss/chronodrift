...
// 【DESIGN 5章】水面ポリゴンがあればその水位を優先する(地形は今や4章の適合カットで水域の
// 内側には存在しない=穴のため、通常はgとwがほぼ同じ意味になるが、境界パッチの縁では
// わずかに違いうる。maxではなく明示的にwを優先する設計に統一する)。
function surfaceY(x, z) {
  const w = waterLevelAt(x, z);
  return w === null ? getGroundY(x, z) : w;
}
