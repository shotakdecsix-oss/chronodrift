...
// 【2026-08-13】150→60→0。このガードは「判定が誤って陸を沈める」のを防ぐためだったが、
// 汀線ランプ(SHORE_RAMP)が陸側ノードを海面近くまで下げるようになったため、誤判定された
// ノードが余分に1.9下がるかどうかの違いしか残らず、守る意味が消えた。
// 一方で「下げないノード」の生DEM(実測で海面より+2.04)が200m先まで補間で効き、汀線沿いに
// 海面をわずかに超える帯を作っていた(実測: ground=20.36 に対し seaY=21.30 で余裕0.94のみ)。
// 0にすることで海と判定されたノードは全て下がり、余裕が約1.9に広がる。
const SEA_BED_INSET = 0;     // 海岸線からこの距離以内のノードは下げない(m)。0=ガード無効
